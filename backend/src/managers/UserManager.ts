...
export class UserManager {
  ...
  private maxNumberOfQueues: number;
  ...
  pushToQueues(user: User) {
    ...
    if (user.queueIndex !== undefined) {
      this.queues[user.queueIndex] = this.queues[user.queueIndex].filter(
  ...
  connectUsers(user1: User, user2: User) {
    /**
  ...
  removeUserFromQueues(id: string, queueIndex: number) {
    this.queues[queueIndex] = this.queues[queueIndex].filter(
      (user) => user.id !== id,
    );
  }
  ...
  removeUser(id: string, queueIndex?: number) {
...
function hasListeners(socket: Socket, eventName: string) {
